...
async def process_link(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer_video(video=open('video.mp4', 'rb'), caption=caption[:1024] if caption else "Video")
        
        if caption:
            hashtags = [w for w in caption.split() if w.startswith('#')]
            if hashtags:
                await message.answer(f"Heshteglar:\n{' '.join(hashtags)}")
            
        if os.path.exists('video.mp4'):
            os.remove('video.mp4')
            
    except Exception as e:
        await message.answer(f"Yuklashda xatolik: {str(e)}")
    
    await state.finish()

# Qolgan barcha tugmalar funksiyalari (Rek heshteg, Nakrutka, 10 do'st...) 
# o'zgarishsiz qoladi, shunchaki ularni shu qatorlardan pastga qo'shib qo'ying:
...
async def reklam_xizmat(message: types.Message):
    await message.answer("Reklama uchun: @roziyev2")
